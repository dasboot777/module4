...
def course(valute_from, valute_to, amount):
    #парсим данные валюты №1 DKK
    valute_from_data = soup.find("Valute", ID="R01215")
    ...
    #
    # # парсим данные валюты №2 GBP
    valute_to_data = soup.find("Valute", ID="R01035")
    ...
    if valute_from == "RUR":
        print(
            f"При конвертации {amount} {valute_from} Российских рублей получится {itogo} {valute_to} {to_currency_name}")
    else:
        print(
            f"При конвертации {amount} {valute_from} {from_currency_name} получится {itogo} {valute_to} {to_currency_name}")
