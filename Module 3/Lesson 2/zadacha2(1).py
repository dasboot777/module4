...
while True:
    ...
    if messages['count'] >= 1:
        ...
        if msg_text.lower() == "корабли":
            ...
            # Выводим имя и скорость самого быстрого корабля
            korabli = max(((x['name'], x['max_atmosphering_speed']) for x in api),
                      key=lambda x: int(x[1].replace('n/a', '0').replace('km', '')))

            ans = f"звёздный корабль с максимальной скоростью: {korabli} "
        else:
