import vk_api
from  vk_api.longpoll import VkLongPoll, VkEventType
import random
# from  cbrf import find_valute
from wiki import get_article
import  requests
from  bs4 import  BeautifulSoup

vk_session = vk_api.VkApi(token="vk1.a.2WKH4g6AdSKYyXjzi0jm08QSLR3kNSzadLJ7rIESIviy39LlRr4G9ta0vcgc9IKaypCoHIP2CdmtB_zwGjDX2hyjNtdKKbPyM4p8bgUlZIajZ9P4976fdV68RneO-Gy603n0A8N9oMj2z0ogIHtDcGjqkncmRNfMzinnI_E4HO91BvtPGiHxkScg9gskNm09_S7AISfN2iPVFF4tclTz0A")
#удалили осла
vk = vk_session.get_api()
longpoll = VkLongPoll(vk_session)

for event in longpoll.listen():#генераттор событий, перебор по событиям
    # print(event)
    if event.type == VkEventType.MESSAGE_NEW and event.to_me:
        user_message = event.text.lower()
        # print(user_message)
        if user_message[0:2] == "-к":


            user_currency = user_message[3:]


            url = "http://www.cbr.ru/scripts/XML_daily.asp"

            response = requests.get(url)
            # print(response)
            # print(response.content)

            soup = BeautifulSoup(response.content, features="xml")
            dict = {}
            # print(dict)
            currencies_list = soup.find_all("Valute")
            for currency in currencies_list:
                currency_name = currency.Name.text.lower()
                currency_value = currency.Value.text
                currency_nominal = currency.Nominal.text
                currency_charcode = currency.CharCode.text
                znachenie = f"({currency_nominal} шт.) {currency_charcode} {currency_name} стоит(ят) {currency_value} рублей"
                # print(znachenie)
                dict[currency.Name.text.lower()] = znachenie
            # print(dict)
            # print(dict[user_currency.lower()])
            try:
                response = f'Вот что я нашел: {dict[user_currency.lower()]}'
            except:
                response = "Ничего не нашлось"


            # print(user_currency)
            # print(user_message)

        elif user_message[0:2] == "-в":
            article = user_message[2:]
            response = f'Вот что я нашел: \n\n{get_article(article)}'

        else:
            response = "Не знаю такой команды"
        vk.messages.send(user_id=event.user_id, message=response, random_id=random.randint(-10 **7, 10 ** 7))


